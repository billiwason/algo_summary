...
int main() {
	...
	iv T_IN(N+1), T_OUT(N+1);

	int timer = 0;

	std::function<void(int,int)> getOtt;

	auto getOtt = [&] (int cur, int parent) -> void {
		T_IN[cur] = ++timer;
	...
	FOR(i, Q)
	{
		...
		if(op == 1)
		{
			...
			ft.update(T_IN[target], weight);
		}
		else
		{
			int target;
			cin >> target;
			auto ans = ft.query(T_IN[target], T_OUT[target]);
			cout << ans << endl;
		}
	}
