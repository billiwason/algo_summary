...
S op(S l, S r) {
	ll new_a = (l.a + r.a) % INF_1E97;

	return S{new_a, l.size + r.size};
}
S e() { return S{0, 0}; }
S mapping(F l, S r) 
{
	ll t1 = ( r.a * l.a ) % INF_1E97;
	ll t2 = (r.size * l.b) % INF_1E97;
	ll new_a = (t1 + t2) % INF_1E97;
	return S{new_a, r.size}; 
}
F composition(F l, F r) 
{
	ll new_a = (r.a * l.a) % INF_1E97;
	ll new_b = (((r.b * l.a) % INF_1E97) + l.b) % INF_1E97;
...
int main() {
	...
	FOR(i, Q)
	{
		...
		else if( op == 3)
		{
			...
			ll v;
			cin >> st >> ed >> v;
			seg.apply(st, ed+1, F{0,v});
		}
		else // op == 4
		{
			int st, ed;
			cin >> st >> ed;
			ll ans = seg.prod(st, ed+1).a;
			cout << ans % INF_1E97 << endl;
		}
	}
    return 0;    
}