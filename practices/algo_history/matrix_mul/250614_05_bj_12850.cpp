...
int main() {
	...
	graph[2][1] = graph[2][3] = graph[2][5] = graph[2][6] = 1;
	graph[3][2] = graph[3][4] = graph[3][6] = 1;
	graph[4][3] = graph[4][7] = 1;
	graph[5][0] = graph[5][1] = graph[5][2] = graph[5][6] = 1;
	graph[6][2] = graph[6][3] = graph[6][5] = graph[6][7] = 1;
	graph[7][4] = graph[7][6] = 1;

	int N;
	cin >> N;

	function<llv2(llv2, int)> getMatPow;

	auto getMatMul = [&] (llv2 a, llv2 b) -> llv2 {
		llv2 ans(8, llv(8));
	...
	getMatPow = [&] (llv2 mat, int pow_) -> llv2 {
		if(pow_ == 1)
			return mat;

		if(pow_ % 2)
		{
			return getMatMul(getMatPow(mat, pow_-1), mat);
		}
		else
		{
			llv2 subAns = getMatPow(mat, pow_/2);
			return getMatMul(subAns, subAns);
		}
	};

	llv2 ans = getMatPow(graph, N);
