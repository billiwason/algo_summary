...
int main() {
	...
	FanWickTree<int> FT(N+1);

	int timer = 0;

	function<void(int,int)> OTT;

	iv tin(N+1);
	iv tout(N+1);

	OTT = [&] (int cur , int parent ) -> void {
		tin[cur] = ++timer;
	...
	OTT(1, -1);
	vector<qInfo> query(Q);

	FOR(i, Q)
	{
		cin >> query[i].node >> query[i].cl;
		query[i].idx = i;
	}

	sort(query.begin(), query.end(), [] (qInfo &a, qInfo &b) {
		return a.cl < b.cl;
	});
	...
	{
		int cur = q.cl;
		for(int k = updateColorIndex+1 ; k <= cur ; k++)
		{
			for(auto &c : cList[k])
			{
				FT.update(tin[c], 1);
		...
		updateColorIndex = cur;
		q.ans = FT.query(tin[q.node], tout[q.node]);
	}
	...
	for(auto &q : query)
	{
		...
		//cout << "test = " << q.idx << ", " << q.node << ", " << q.cl << ", " << q.ans << endl;
	}
	cout << ans << endl;
