...
int main() {
	...
	// n : 전체 수열개수
	// k : 아름다운 수열을 만들 수 있는 최소 개수
	int N, K; 
	cin >> N >> K;

	// prefix sum 을 계산하려면 인덱스는 1 base 인 것이 좋다.
	iv arr(N+1);
	...
	// 아름다운 수가 나올수 있는 범위는 1 부터 수열의 최대값

	// 0 ~ maxVal 사이를 이진탐색하면서 해당 값이 수열을 구성할 수 있는 값인지 찾는다.
	...
	auto check = [&] (double x) -> bool {
		...
		for(int i = 1 ; i <= N ; i++)
		{
			acc += (double)arr[i] * arr[i] - x * arr[i];
			prefix.push_back(acc);
		}
		double minPreFix = 0;
		// k 개 이상의 구간만 판단하기 때문이다.
		for(int j = K ; j <= N ; j++)
		{
    ...
    while (low <= high) {
		...
		// 만약 해당 값이 아름다운 수열 값이면 mid 를 더 증가시켜 볼 수 있다.
        if (check(mid)) 
		{
            low = mid;
			ans = max(ans , mid);
