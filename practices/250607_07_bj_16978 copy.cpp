...
struct Query {
	int st;
	int ed;
	int uOrder; // 몇번째 쿼리 까지 업데이트 된 후 계산해야 하는가, 0 이면 없데이트 없이 쿼리함.
	ll ans;
	int index;
};
...
int main() {
	...
	FOR(i, Q)
	{
		...
		if(op == 1)
		{
			int a,b;
			cin >> a >> b;
			updateList.push_back(make_pair(a,b));
		}
		else
		{
			int ord, st, ed;
			cin >> ord >> st >> ed;
			qry.push_back(Query{st, ed, ord, 0, (int)qry.size()});
		}
	}
	// query 를 각 Query 를 수행하기 전 필요한 update index 순으로 정렬한다.
	...
	for(auto &q : qry)
	{
		for(int i = lastUpdated+1 ; i < q.uOrder ; i++)
		{
			//cout << "f/s =" << updateList[i].first << "," << updateList[i].second << endl;
			//cout << "Q.order =" << q.uOrder << endl;
			seg.set(updateList[i].first, updateList[i].second);
		}
		lastUpdated = q.uOrder-1;

		q.ans = seg.prod(q.st, q.ed+1);
	}
	// 출력을 위해 index 순으로 다시 정렬한다.
	sort(qry.begin(), qry.end(), [](Query &a, Query &b) -> bool {
