...
int main() {
	...
	int direction = 0; // 0 : 내리 칭찬, 1: 오름 칭찬

	FanWickTree<int> FT_down(N*2), FT_up(N*2);
	...
	FOR(i, Q)
	{
		...
		if(op == 1)
		{
			...
			if(direction == 0)
			{
				FT_down.update(tin[cur], wieght);
				FT_down.update(tout[cur], -wieght);
			}
			else
			{
				FT_up.update(tin[cur], wieght);
			}
		}
		else if(op == 2)
		{
			int cur;
			cin >> cur;
			int sub1 = FT_down.query(1, tin[cur]);
			int sub2 = FT_up.query(tin[cur], tout[cur]);
