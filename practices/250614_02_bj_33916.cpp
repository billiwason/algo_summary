...
void solve(llv &primes, iv2 &DP)
{
	...
	auto updateN = [&] (int n , int mul, iv &ans) -> void {
		FOR(i, sz)
		{
			ans[i] += DP[n][i] * mul;
		}
	...
	auto updateCnt = [&] (int n , int r , iv &ans) -> void {
		updateN(n , 1, ans);
		updateN(r, -1, ans);
		updateN(n-r, -1, ans);
		return ;
	};
	updateCnt(n1, r1, cnt1);
	updateCnt(n2, r2, cnt2);
...
int main() {
	...
	llv pNumber = getPrimeNumber(5001);

	int sz = pNumber.size();

	int numLim = 5000;

	iv2 DP(numLim+1, iv(sz));


	auto updateN = [&] (int n , iv &ans) -> void {
		while(n > 1)
		{
			FOR(i, sz)
			{
