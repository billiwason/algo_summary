...
int main() {
	...
	FOR(r, R)
	{
		...
		//printDPTable("preFix =" , prefix);

		llv l_max(C+1); // 1~ j 까지의 중 최대값.
		for(int i = 1 ; i <= C ; i++)
		{
			l_max[i] = max(l_max[i-1] + arr[i] , arr[i]);
			l_max[i] = max(0LL, l_max[i]);
		}
		llv r_max(C+2);
		...
		for(int i = C ; i >= 1 ; i--)
		{
			...
			{
				r_max[i] = max(r_max[i+1] + arr[i], arr[i]);
			}
			r_max[i] = max(0LL, r_max[i]);
		}
		llv DP_next(C+1, -INF_1E187);

		ll best = -INF_1E187;
		for(int i = 1; i <= C ; i++)
		{
			best = max(best, DP_prev[i] + l_max[i-1] - prefix[i-1]);
			DP_next[i] = max(DP_next[i], best + prefix[i] + r_max[i+1]);
		}

		best = -INF_1E187;
		for(int i = C ; i >= 1 ; i--)
		{
			best = max(best, DP_prev[i] + r_max[i+1] + prefix[i]);
			DP_next[i] = max(DP_next[i], best - prefix[i-1] + l_max[i-1]);
		}
		//printDPTable("DP_next" , DP_next);
		DP_prev = DP_next;
	}
