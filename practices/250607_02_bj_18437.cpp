...
int main() {
	...
	int timer = 0;
	function<void(int ,int)> ETT;
	iv tin(N+1), tout(N+1);
	ETT = [&] (int cur, int parent) -> void {
		tin[cur] = ++timer;
	...
	FOR(i, Q)
	{
		...
		if(op == 1) // on pc
		{
			seg.apply(tin[cur]+1, tout[cur]+1, F{0,1});
		}
		else if(op == 2) // off pc
		{
			seg.apply(tin[cur]+1, tout[cur]+1, F{0,0});
		}
		else // op == 3
		{
			ll ans = seg.prod(tin[cur]+1, tout[cur]+1).a;
			cout << ans << endl;
		}
	}
    return 0;    
