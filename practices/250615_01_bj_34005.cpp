...
int solve()
{
	// 경과시간/ SP / 공통 쿨 / 스트라이크 쿨 // 박치기 쿨쿨
	iv5 DP(610, iv4(110, iv3(3, iv2(4, iv(9,-1)))));
	...
	auto maxClip = [] (int sp) -> int{
		return min(100, sp);
	};

	auto minClip = [] (int n) -> int{
		return max(0, n);
	};

	auto dpUpdate = [&] (int &next, int newVal) -> void {
		newVal = max(0, newVal);
		if(next == -1)
			next = newVal;
		next = min(next, newVal);
	};

	for(int i = 0 ; i <= 600 ; i ++) // 경과 시간
	{
		for(int j = 0 ; j <= 100 ; j++) // SP
		{
			for(int k = 0 ; k < 3 ; k++) // 공통 쿨
			{
				for(int l = 0 ; l < 4 ; l++) // 스트라이크 쿨
				{
					for(int m = 0 ; m < 9 ; m++)
					{
						if(DP[i][j][k][l][m] == -1)
							continue;
						int cur = DP[i][j][k][l][m];
						// 아무 공격도 안할 수 있다.
						dpUpdate(DP[i+1][maxClip(j+1)][minClip(k-1)][minClip(l-1)][minClip(m-1)] , cur);

						if(k == 0) // 공통쿨이 0 일때만 공격 가능능
						{
							// 무조건 기본공격은 할 수 있다.
							if(cur - 100 <= 0)
								return i;

							dpUpdate(DP[i+1][maxClip(j+1)][0][minClip(l-1)][minClip(m-1)] , cur - 100);
							// 헤머링도 바로 할 수 있다.
							if(cur - A <= 0)
							{
								return i;
							}
							dpUpdate(DP[i+1][maxClip(j+8)][0][minClip(l-1)][minClip(m-1)] , cur - A);
							
							if(l == 0 && j >= 10) // 스트라이크 공격 가능, sp 가 10 보다 커야 한다.
							{
								if(cur - B <= 0)
									return i;
								
								dpUpdate(DP[i+1][minClip(j-9)][1][2][minClip(m-1)] , cur - B);	
								
							}
							if(m == 0 && j >= 10) // 박치기 공격 가능능
							{
								if(cur - C <= 0)
									return i;
								//cout << "i =" << i << ", hp = " << cur-A << endl;
								dpUpdate(DP[i+1][minClip(j-9)][1][minClip(l-1)][7] , cur - C);
							}
