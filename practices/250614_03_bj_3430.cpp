...
void solve()
{
	...
	iv last(nrLake+1); // 각 호수별로 마지막 비가 내린 날.

	set<int> eatable_days;
	...
	for(int d = 1 ; d <= nDay ; d++)
	{
		...
		if(t == 0)
		{
			eatable_days.emplace(d);
			visit[d] = 0;
		}
		else
		{
			auto getDay = eatable_days.lower_bound(last[t]);

			if(getDay == eatable_days.end())
			{
				cout << "NO" << endl;
				return ;
			}
